...
template <typename ADataType,
          ...
          typename BElementOp   = ck_tile::identity,
          typename ACCElementOp = ck_tile::identity>
CK_TILE_HOST void reference_gemm_scale(const HostTensor<ADataType>& a_m_k,
                                       const HostTensor<BDataType>& b_k_n,
                                       HostTensor<CDataType>& c_m_n,
                                       const HostTensor<CDataType>& a_scale_m_k128,
                                       const HostTensor<CDataType>& b_scale_k128_n128,
                                       const AElementOp& a_element_op     = {},
                                       const BElementOp& b_element_op     = {},
                                       const ACCElementOp& acc_element_op = {})
{
    ...
    auto f_mn = [&](auto m, auto n) {
        ...
        for(std::size_t k = 0; k < K; ++k)
        {
            ...
            const std::size_t k_scale_idx = k / 128;
            const std::size_t n_scale_idx = n / 128;
            const float a_s = a_scale_m_k128(m, k_scale_idx);
            const float b_s = b_scale_k128_n128(k_scale_idx, n_scale_idx);

            v_acc += v_a * v_b * a_s * b_s;
