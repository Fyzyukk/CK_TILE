                                                 ...
                                                 TopDimensionHiddenIds>
{
    ...
    constexpr static index_t ntransform_  = Base::get_num_of_transform();//1
    constexpr static index_t ndim_hidden_ = Base::get_num_of_hidden_dimension();//3->0,1,2
    constexpr static index_t ndim_top_    = Base::get_num_of_top_dimension();//2->1,2
...
{
    constexpr index_t N = sizeof...(Lengths);//2

    const auto transforms = make_tuple(make_embed_transform(lengths, strides)); //[M,K/128]+[K/128,1]

    constexpr auto low_dim_hidden_idss = make_tuple(sequence<0>{});//tuple(0)

    constexpr auto up_dim_hidden_idss =
        make_tuple(typename arithmetic_sequence_gen<1, N + 1, 1>::type{});//tuple(1,2)
