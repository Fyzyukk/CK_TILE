                                                                  ...
                                                                  const TopIndex& idx_top)
{
    ...
    constexpr index_t ntransform  = Adaptor::get_num_of_transform();//1
    constexpr index_t ndim_hidden = Adaptor::get_num_of_hidden_dimension();//3
    constexpr auto bottom_dim_ids = Adaptor::get_bottom_dimension_hidden_ids();//0
    constexpr auto top_dim_ids    = Adaptor::get_top_dimension_hidden_ids();//1,2
    ...
    //idx_hidden<0,0,0>+<1,2>+<x,y>---->idx_hidden<0,x,y>

    // calculate hidden index
    static_for<ntransform, 0, -1>{}([&adaptor, &idx_hidden](auto itran_p1) {
        auto itran              = itran_p1 - number<1>{};//0
        const auto& tran        = adaptor.get_transforms().at(itran); //{[M,K/128], [K/128,1]}
        ...
        //将idx_hidden<0,x,y>的<0>位置变成idx_low---><x*stride[0]+y*stride[1],x,y>
    });
