...
struct tile_distribution
{
    ...
    //sequence<1>, tuple<sequence<Y0,Y1, Y2>, sequence<X0, X1>
    //Ps维度: tuple<sequence<1>, sequence<1, 2>>,
    ...
    //返回的是tuple<sequence<Y1, Y2>, sequence<X0, X1>
    //每个X维度的大小，相当于就是每个warp处理大小[Y1*Y2,X0*X1]
    ...
    CK_TILE_HOST_DEVICE auto calculate_rs_index_from_ps_index(const PartitionIndex& ps_idx) const
    {
        ...
        static_for<0, NDimP, 1>{}([&](auto idim_p) {
            ...
            //idim_p=0 ps_to_rhss_major_[0].size() Y0 Warp数量 = 1
        ...
        get_y_indices_from_distributed_indices(DistributedIndices)
    {
        ...
        //sequence<ys_idx_arr[0],ys_idx_arr[1]>
    }
    //获取Y维度的坐标。
    ...
    make_adaptor_encoding_for_tile_distribution(StaticTileDistributionEncoding_)
{
    ...
    //rh_major_minor_to_hidden_ids[0][0] = 2
    //rh_major_minor_to_hidden_lengths[0][0] = Y0
    //trans[0] = {replicate,{Y0},0,{},1,{2}}
    ...
    //rh_major_minor_to_hidden_ids[1][0] = 3
    //rh_major_minor_to_hidden_lengths[1][0] = Y1
    //rh_major_minor_to_hidden_ids[1][1] = 4
    //rh_major_minor_to_hidden_lengths[1][1] = Y2
    //trans[1] = {unmerge,{Y1,Y2},1,{0},2,{3,4}}
    //rh_major_minor_to_hidden_ids[2][0] = 5
    //rh_major_minor_to_hidden_lengths[2][0] = X0
    //rh_major_minor_to_hidden_ids[2][1] = 6
    //rh_major_minor_to_hidden_lengths[2][1] = X1
    //trans[2] = {unmerge,{X0,X1},1,{1},2,{5,6}}
    ...
    static_for<0, ndim_p, 1>{}([&](auto iDimP) {
        ...
        //hidden_dim_id_ps(0)=7
        //hidden_dim_id_ps(1)=8
    ...
    //trans[3] = {merge,{Y0},1,{2},1,{7}}
    //trans[4] = {merge,{Y1,X0},2,{3,5},1,{8}}

    constexpr index_t ndim_bottom = ndim_x; //2
    ...
    constexpr index_t ndim_y   = Ys2RHsMajor::size();//2
    constexpr index_t ndim_top = ndim_p + ndim_y; //4

    auto top_dim_ids = hidden_dim_id_ps; //[7,8,4,6]
    ...
    Lengths y_lengths;//[Y2,X2]
    index_t d_length = 1;//[Y2*X2]
...
CK_TILE_HOST_DEVICE constexpr auto make_tile_distribution(StaticTileDistributionEncoding_)
{
    ...
    constexpr auto ys_to_d_adaptor = 
        CONSTRUCT_TENSOR_ADAPTOR_FROM_ENCODING(ys_to_d_adaptor_impl);

    constexpr auto ys_to_d_descriptor =
        make_tensor_descriptor_from_adaptor(ys_to_d_adaptor, d_length);

    constexpr index_t ndim_rh_major = DstrEncode::detail::ndim_rh_major_;//[3]
    constexpr auto ndims_rhs_minor  = DstrEncode::detail::ndims_rhs_minor_;//[1,2,2]
