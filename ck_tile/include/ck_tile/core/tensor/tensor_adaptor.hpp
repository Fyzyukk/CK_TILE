...
struct tensor_adaptor
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto initialize_element_size(const Transforms& transforms)
    {
        const auto lengths = generate_tuple(
            [&](auto idim_top) {
                ...
                //idim_top=0,idim_hidden=1
                //idim_top=1,idim_hidden=2

                constexpr auto tmp = get_transform_and_its_upper_dimension(number<idim_hidden>{});
                //(0,0,true)
                //(0,1,true)

                constexpr index_t itran   = tmp[number<0>{}];//0/0
                constexpr index_t idim_up = tmp[number<1>{}];//0/1
                constexpr bool found      = tmp[number<2>{}];//true
        ...
        get_transform_and_its_upper_dimension(number<IDimHidden>)
    {
        ...
        static_for<0, ntransform_, 1>{}([&](auto itran) {
            ...
            //up_dim_ids=UpperDimensionHiddenIdss[0]=<1,2>

            static_for<0, up_dim_ids.size(), 1>{}([&](auto idim_up) {
                if constexpr(up_dim_ids[idim_up] == IDimHidden) //up_dim_ids[0]==1,up_dim_ids[1]==2
                {
                    itran_found   = itran; //0 0
                    idim_up_found = idim_up; //0 1
                    found         = true; //true
                }
            });
        });
    ...
    constexpr static index_t ntransform_  = get_num_of_transform(); //1
    constexpr static index_t ndim_hidden_ = get_num_of_hidden_dimension(); //3
    constexpr static index_t ndim_bottom_ = get_num_of_bottom_dimension();//1
    constexpr static index_t ndim_top_    = get_num_of_top_dimension(); //2
    ...
    CK_TILE_HOST_DEVICE constexpr auto calculate_bottom_index(const TopIdx& idx_top) const
    {
        ...
        constexpr index_t ntransform  = get_num_of_transform();//1
        constexpr index_t ndim_hidden = get_num_of_hidden_dimension();//3
        ...
        //idx_hidden<0,0,0> <1,2> idx_top<m,k>---->idx_hidden<0,m,k>
        ...
        static_for<ntransform, 0, -1>{}([&](auto itran_p1) { 
            ...
            //计算m,k对应算出来idx_low是多少

            set_container_subset(idx_hidden, dims_low, idx_low);
            //然后再将idx_hidden<0,m,k>的0位置 值变为idx_low
        });
        ...
        const array<index_t, ndim_hidden_>& guaranteed_vector_strides)
    {
        ...
        static_for<0, get_num_of_transform(), 1>{}([&](auto itran) { 
            ...
            if constexpr(up_dims.size() > 0)
            {
                for(index_t i = 0; i < up_dims.size(); ++i)
                {
                    up_vector_lengths(i) = (up_guaranteed_vector_lengths[i] != -1)
                                               ...
                                               : up_vector_lengths[i];
                    //up_vector_lengths(0) = up_guaranteed_vector_lengths[0]

                    up_vector_strides(i) = (up_guaranteed_vector_strides[i] != -1)
                                               ? up_guaranteed_vector_strides[i]
                                               : up_vector_strides[i];
                    //up_vector_stride(0) = up_guaranteed_vector_stride[0]
                }
            }
            ...
            //将输入<x,y,z> 的<1,2>位置的值变成up_vector_lengths，up_vector_striide
        });
