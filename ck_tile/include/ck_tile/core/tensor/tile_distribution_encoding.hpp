...
struct tile_distribution_encoding
{
    ...
    struct detail
    {
        ...
        static constexpr index_t ndim_rh_major_   = NDimX + 1; //Hs+Rs //3
        static constexpr index_t ndim_span_major_ = NDimX; //2

        // ndims_rhs_minor_[ndim_rh_major_]
        static constexpr auto ndims_rhs_minor_ = generate_array( //Hs+Rs每个的size
            [](auto i) {
        ...
        // max_ndim_rh_minor_ Hs+Rs的每个size中的最大值
        static constexpr index_t max_ndim_rh_minor_ = 
            container_reduce(ndims_rhs_minor_, maximize<index_t>{}, 0);

        // rhs_lengthss_[ndim_rh_major_][max_ndim_rh_minor_] 外层是R+H的size，内层是size的最大数
        ...
        // ys_lengths_ NDimY=Ys::size() 这一步是从ys到R+H的对应。
        ...
        // rhs_major_minor_to_ys_[ndim_rh_majpr_][max_ndim_rh_minor_] //上面的反变换
        ...
        // ndims_span_minor_[NDimX] NDimX=HsLengths::size HsLength每个长度中几个对应使用
