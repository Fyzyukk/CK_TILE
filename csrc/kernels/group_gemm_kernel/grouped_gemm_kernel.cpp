...
float grouped_gemm_impl(const std::vector<grouped_gemm_kargs>& gemm_descs,
    ...
    using TilePartitioner = ck_tile::GemmSpatiallyLocalTilePartitioner<
        GemmShape, TileParitionerGroupNum, TileParitionerM01>;

    using Traits = ck_tile::TileGemmTraits<kPadM, kPadN, kPadK, ALayout, BLayout, CLayout>;
    using GemmUniversalTraits = ck_tile::TileGemmUniversalTraits<
        kPadM, kPadN, kPadK, DoubleSmemBuffer, ALayout, BLayout, CLayout, TransposeC>;
    ...
    const auto Run = [&](const auto has_hot_loop_,
        ...
        using GemmEpilogue = ck_tile::CShuffleEpilogue<
            ck_tile::CShuffleEpilogueProblem<ADataType, BDataType, DsDataType, AccDataType,
                                            CDataType, DsLayout, CLayout, CDEElementWise,
                                            GemmPipelineProblem::kBlockSize,
                                            TilePartitioner::MPerBlock, TilePartitioner::NPerBlock,
                                            M_Warp, N_Warp, M_Warp_Tile, N_Warp_Tile, K_Warp_Tile,
                                            UniversalGemmProblem::TransposeC, memory_operation>>;
        ...
        hipError_t hip_error = hipMemcpyWithStream(kargs_ptr, kargs.data(),
                                                  get_workspace_size(gemm_descs),
                                                  hipMemcpyHostToDevice, s.stream_id_);
        if (hip_error != hipSuccess) {
            throw std::runtime_error("Failed to copy kernel arguments to device");
        }

        if (s.log_level_ > 0) {
            std::cout << "Launching kernel: " << Kernel::GetName() 
                      << " with args: grid: {" << grids.x << ", " << grids.y << ", " << grids.z << "}"
...
extern "C" float grouped_gemm_fp16_fp16_fp16(
    ...
    return grouped_gemm_impl<ADataType, BDataType, DsDataType, AccDataType, CDataType,
                            ALayout, BLayout, DsLayout, CLayout,
                            ck_tile::element_wise::PassThrough>(gemm_descs, s, kargs_ptr);
}
